...
from tensordata import string_int_label_map_pb2
...
def get_label_map_dict(
    label_map_path, use_display_name=False, fill_in_gaps_and_background=False
):
  ...
  """
    ...
    if fill_in_gaps_and_background:
        ...
        if not all(isinstance(value, int) for value in values):
            raise ValueError(
                "The values in label map must be integers in order to"
