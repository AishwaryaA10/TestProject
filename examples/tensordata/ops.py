...
from tensordata import standard_fields as fields
from tensordata import shape_utils
from tensordata import static_shape
...
def normalized_to_image_coordinates(
    normalized_boxes, image_shape, parallel_iterations=32
):
  ...
  """
    ...
    if tensor_height is None:
        ...
        padded_tensor_height = (
            tf.to_int32(tf.ceil(tf.to_float(tensor_height) / tf.to_float(multiple)))
            * multiple
        )
    else:
    ...
    if tensor_width is None:
        ...
        padded_tensor_width = (
            tf.to_int32(tf.ceil(tf.to_float(tensor_width) / tf.to_float(multiple)))
            * multiple
        )
    else:
    ...
    if padded_tensor_height != tensor_height:
        height_pad = tf.zeros(
            [
                batch_size,
                padded_tensor_height - tensor_height,
                tensor_width,
                tensor_depth,
            ]
        )
...
def padded_one_hot_encoding(indices, depth, left_pad):
  ...
  """
    ...
    def one_hot_and_pad():
        one_hot = tf.cast(
            tf.one_hot(tf.cast(indices, tf.int64), depth, on_value=1, off_value=0),
            tf.float32,
        )
        return tf.pad(one_hot, [[0, 0], [left_pad, 0]], mode="CONSTANT")

    result = tf.cond(
        tf.greater(tf.size(indices), 0),
        one_hot_and_pad,
...
def dense_to_sparse_boxes(dense_locations, dense_num_boxes, num_classes):
  ...
  """
    ...
    box_locations = tf.slice(
        dense_locations, tf.constant([0, 0]), tf.stack([num_valid_boxes, 4])
    )
    tiled_classes = [
  ...
  """
    ...
    if fields.InputDataFields.groundtruth_boxes in tensor_dict:
        ...
        num_boxes = tf.maximum(
            tf.shape(tensor_dict[fields.InputDataFields.groundtruth_boxes])[0], 1
        )
        for key in tensor_dict:
            ...
            # Needs to reshape into [num_boxes, -1] for tf.gather() to work.
                ...
                valid_dict[key] = tf.reshape(
                    tf.gather(
                        tf.reshape(tensor_dict[key], [num_boxes, -1]), valid_indices
                    ),
                    [-1],
                )
...
def filter_groundtruth_with_nan_box_coordinates(tensor_dict):
  ...
  """
    ...
    nan_indicator_vector = tf.greater(
        tf.reduce_sum(tf.to_int32(tf.is_nan(groundtruth_boxes)), reduction_indices=[1]),
        0,
    )
...
def normalize_to_target(
  ...
  """
    with tf.variable_scope(scope, "NormalizeToTarget", [inputs]):
        ...
        if not (
            isinstance(target_norm_value, float)
            or (isinstance(target_norm_value, list) and len(target_norm_value) == depth)
            and all([isinstance(val, float) for val in target_norm_value])
        ):
            raise ValueError(
...
def batch_position_sensitive_crop_regions(
    images, boxes, crop_size, num_spatial_bins, global_pool, parallel_iterations=64
):
...
def position_sensitive_crop_regions(
    image, boxes, crop_size, num_spatial_bins, global_pool
):
  ...
  """
    ...
    for (split, box) in zip(image_splits, position_sensitive_boxes):
        if split.shape.is_fully_defined() and box.shape.is_fully_defined():
            crop = matmul_crop_and_resize(tf.expand_dims(split, 0), box, bin_crop_size)
        else:
            crop = tf.image.crop_and_resize(
                tf.expand_dims(split, 0),
                box,
  ...
  """
    with tf.name_scope("nearest_neighbor_upsampling"):
        ...
        output_tensor = tf.reshape(
            input_tensor, [batch_size, height, 1, width, 1, channels]
        ) * tf.ones([1, 1, scale, 1, scale, 1], dtype=input_tensor.dtype)
        return tf.reshape(
            output_tensor, [batch_size, height * scale, width * scale, channels]
        )
...
def matmul_crop_and_resize(image, boxes, crop_size, scope=None):
  ...
  """
    ...
    if len(img_shape) != 4 and img_shape[0] != 1:
        raise ValueError(
            "image should have shape " "`[1, image_height, image_width, depth]`"
        )
    ...
    def _lin_space_weights(num, img_size):
        if num > 1:
            ...
            start_weights = num * [0.5 * (img_size - 1)]
            stop_weights = num * [0.5 * (img_size - 1)]
        return (
            tf.constant(start_weights, dtype=tf.float32),
...
def expected_classification_loss_under_sampling(
    batch_cls_targets, cls_losses, negative_to_positive_ratio, minimum_negative_sampling
):
  ...
  """
    ...
    # find the p_i
    foreground_probabilities = foreground_probabilities_from_targets(batch_cls_targets)
    ...
    beta = tf.where(tf.equal(beta, 0), min_negative_weight * tf.ones_like(beta), beta)
    beta = tf.reshape(beta, [-1, 1])

    cls_loss_weights = foreground_probabilities + (1 - foreground_probabilities) * beta
